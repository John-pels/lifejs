...
export class LifeApi {
  ...
  async start() {
    using h0 = (await this.server.telemetry.trace("start()")).start();
  ...
  async stop() {
    using _ = (await this.server.telemetry.trace("stop()")).start();
  ...
  async handleRequest({
    ...
    // Telemetry
    using h0 = (await this.server.telemetry.trace("handleRequest()", { inputStr })).start();
    ...
    const sanitizeResult = (result: op.OperationResult<unknown>) => {
      ...
      const [errorCanon, response] = canon.stringify(resultPublic as unknown as SerializableValue);
      if (errorCanon)
    ...
    const prepareResponse = (result: op.OperationResult<unknown>) => {
      ...
      this.server.telemetry.log.info({
        message: `${type.toUpperCase()} ${error ? error.code : "Valid"} ${type === "http" ? `(${statusChalk.bold(status)})` : ""} ${handlerId} [in ${chalk.bold(`${ns.toMs(h0.getData().duration)}ms]`)}`,
      });
