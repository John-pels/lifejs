...
export class LifeCompiler {
  ...
  async start() {
    return await this.telemetry.trace("start()", async (span) => {
      try {
        ...
        span.log.debug({ message: `Optimized build: ${this.options.optimize}` });
        ...
        if (!this.entryPaths.servers.length) {
          span.log.info({
            message:
              "Nothing to compile yet. Create a first `agent/server.ts` file in your project.",
          });
          return op.success();
        }
  ...
  async initEntryPaths() {
    return await this.telemetry.trace("initEntryPaths()", async (span) => {
  ...
  watchEntryPaths() {
    ...
    const processWatchEvent = async (action: "added" | "removed" | "changed", relPath: string) => {
      return await this.telemetry.trace("processWatchEvent()", async (span) => {
        ...
        // Process server paths
          ...
          if (action === "added") {
            ...
            this.hashes.set(absPath, await this.hashFile(absPath));
            // Compile the server
            await this.telemetry.trace("compile-server", async (spanCompile) => {
              spanCompile.setAttributes({ serverPath: absPath });
              ...
              spanCompile.end();
              span.log.info({
                message: `Agent server '${chalk.bold.italic(name)}' re-compiled in ${chalk.bold(`${ns.toMs(spanCompile.getData().duration)}ms`)}.`,
          ...
          // - Changed
            ...
            await refreshEntryPathDependencies(absPath);
            // Compile the server
            await this.telemetry.trace("recompile-server", async (spanRecompile) => {
              spanRecompile.setAttributes({ serverPath: absPath });
              const { needBundle, name } = await this.compileAgentServer(absPath);
              if (needBundle) await this.generateServerBundle();
              ...
              spanRecompile.end();
              span.log.info({
                message: `Agent server '${chalk.bold.italic(name)}' re-compiled in ${chalk.bold(`${ns.toMs(spanRecompile.getData().duration)}ms`)}.`,
                attributes: { serverPath: absPath },
              });
            });
          }
        ...
        // Process client paths
          ...
          if (action === "added") {
            ...
            this.hashes.set(absPath, await this.hashFile(absPath));
            // Compile the client
            await this.telemetry.trace("compile-client", async (spanCompile) => {
              spanCompile.setAttributes({ clientPath: absPath });
              ...
              spanCompile.end();
              span.log.info({
                message: `Agent client '${chalk.bold.italic(name)}' re-compiled in ${chalk.bold(`${ns.toMs(spanCompile.getData().duration)}ms`)}.`,
          ...
          // - Changed
            ...
            await refreshEntryPathDependencies(absPath);
            // Compile the client
            await this.telemetry.trace("recompile-client", async (spanRecompile) => {
              spanRecompile.setAttributes({ clientPath: absPath });
              const { needBundle, name } = await this.compileAgentClient(absPath);
              if (needBundle) await this.generateClientBundle();
              ...
              spanRecompile.end();
              span.log.info({
                message: `Agent client '${chalk.bold.italic(name)}' re-compiled in ${chalk.bold(`${ns.toMs(spanRecompile.getData().duration)}ms`)}.`,
                attributes: { clientPath: absPath },
              });
            });
          }
        ...
        // Process dependency paths
          ...
          await this.telemetry.trace("recompile-touched-paths", async (spanRecompile) => {
            spanRecompile.setAttributes({ touchedPaths });
            await Promise.all(
            ...
            if (recompiledAgentServers.size > 0) {
              span.log.info({
                message: `Agent${recompiledAgentServers.size > 1 ? "s" : ""} server${recompiledAgentServers.size > 1 ? "s" : ""} ${formattedServerNames} re-compiled in ${chalk.bold(`${ns.toMs(spanRecompile.getData().duration)}ms`)}.`,
                attributes: { touchedPaths },
              });
            }
            if (recompiledAgentClients.size > 0) {
              span.log.info({
                message: `Agent${recompiledAgentClients.size > 1 ? "s" : ""} client${recompiledAgentClients.size > 1 ? "s" : ""} ${formattedClientNames} re-compiled in ${chalk.bold(`${ns.toMs(spanRecompile.getData().duration)}ms`)}.`,
                attributes: { touchedPaths },
              });
            }
          });
  ...
  async compileConfig(configPath: string, recompileAffectedAgents = true) {
    await this.telemetry.trace("compileConfig()", async (span) => {
      span.setAttributes({ configPath });
      ...
      try {
        ...
        if (recompileAffectedAgents) {
          await this.telemetry.trace("recompile-affected-agents", async (spanRecompile) => {
            spanRecompile.setAttributes({ configPath });
            ...
            span.setAttributes({ affectedServers, affectedClients });
            ...
            // Log the results
            const names = new Set(
              results.map((result) => result?.name).filter((name) => name !== null),
            );
            const formattedNames = Array.from(names)
              .map((name) => `'${chalk.bold.italic(name)}'`)
              .join(", ");
            span.log.info({
              message: `Agent${names.size > 1 ? "s" : ""} ${formattedNames} re-compiled in ${chalk.bold(`${ns.toMs(spanRecompile.getData().duration)}ms`)}.`,
  ...
  async compileAgentServer(serverPath: string) {
    return await this.telemetry.trace("compileAgentServer()", async (span) => {
      span.setAttributes({ serverPath });
      try {
  ...
  async compileAgentClient(clientPath: string) {
    return await this.telemetry.trace("compileAgentClient()", async (span) => {
      span.setAttributes({ clientPath });
  ...
  async generateServerBundle() {
    return await this.telemetry.trace("generateServerBundle()", async (span) => {
      try {
  ...
  `.trim();
        ...
        span.log.error({ message: "Failed to generate server bundle. Unexpected error.", error });
      }
    });
  }

  async generateClientBundle() {
    return await this.telemetry.trace("generateClientBundle()", async (span) => {
      try {
